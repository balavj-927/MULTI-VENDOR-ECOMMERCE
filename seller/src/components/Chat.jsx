...
const Chat = () => {
  ...
  useEffect(() => {
    ...
    newSocket.on('receiveMessage', ({ sender, message }) => {
      console.log('Seller received message:', { sender, message });
      if (sender === 'admin' || sender === 'seller') { // Only show messages from admin or seller
        setMessages((prevMessages) => {
          ...
          console.log('Seller updated messages:', updatedMessages);
  ...
  const sendMessage = () => {
    if (message.trim() && socket) {
      console.log('Seller sending message to admin:', { sender: 'seller', recipient: 'admin', message });
      socket.emit('sendMessage', { sender: 'seller', recipient: 'admin', message });
      setMessage('');
    }
  };
          ...
          >
            <div
              className={`max-w-xs md:max-w-md px-2 py-1 rounded-lg ${
                msg.sender === 'seller'
                  ? 'bg-gradient-to-br from-blue-900 to-blue-900 text-white rounded-tr-none border border-blue-900'
                  : 'bg-blue-800 text-white rounded-tl-none border border-blue-300'
              } shadow-sm`}
            >
