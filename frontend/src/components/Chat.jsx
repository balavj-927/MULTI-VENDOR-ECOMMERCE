...
const Chat = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-blue-900 border-4 border-navy-900 min-h-96">
      {/* Header with seller info */}
      <div className="flex items-center p-2 text-white shadow-md bg-gradient-to-r from-navy-900 to-navy-800">
        <div className="flex items-center justify-center w-8 h-8 mr-2 bg-white border-2 rounded-full shadow-inner border-navy-900">
...
>
  ...
  <p className={`text-right text-xs ${msg.sender === 'user' ? 'text-blue-200' : 'text-white'} mt-1`}>
    ...
    {msg.sender === 'user' && (
      ...
      {/* Message input area */}
      <div className="flex items-center p-2 bg-navy-100">
        <div className="flex items-center flex-1 px-3 py-1 mr-2 bg-white border rounded-full shadow-sm border-navy-400">
          <input
            type="text"
            value={message}
          ...
          />
          <button className="mx-1 text-lg text-navy-500 hover:text-navy-700">😊</button>
          <button className="mx-1 text-lg text-navy-500 hover:text-navy-700">📎</button>
        </div>
        <button 
          onClick={sendMessage}
          className="flex items-center justify-center w-8 h-8 text-white transition-shadow duration-300 border rounded-full shadow-sm border-navy-700 bg-gradient-to-r from-navy-900 to-navy-800 hover:shadow-md focus:outline-none"
        >
